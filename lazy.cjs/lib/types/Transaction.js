...
class Transaction {
  ...
  setSource(provider) {
    this.source = provider ? (
      provider instanceof types.Provider ? provider : new types.Provider(provider, this.context)
    ) : undefined
    return this
  }
  ...
  setView(view) {
    this.view = view ? (
      view instanceof types.PaymentView ? view : new types.PaymentView(view)
    ) : undefined
    return this
  }
...
Transaction.fromParams = (params = {}, context) => {
  ...
  if (data.source) {
    data.source = types.Provider.fromParams(data.source, context)
  }
  if (data.provider) {
    data.provider = types.Provider.fromParams(data.provider, context)
  }
  if (data.status) {
    let status
    switch (data.status) {
  ...
  if (data.view) {
    data.view = types.PaymentView.fromParams(data.view)
  }
  if (data.date) {
